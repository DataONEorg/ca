...
generates all files needed for the CA, including certificate requests, keys,
...
of the Production CA and its certificates should not be accepted in the
...
The private keys associated with the three DataONE certificate authorities are
not stored online, nor are they made available on the network. To use the
keys, you must have access to the physical media on which the keys reside,
